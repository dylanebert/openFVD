...
int section::exportSection(fstream* file, mnode* anchor, float mPerNode, float fHeart, glm::vec3& vHeartLat, glm::vec3& Norm,
                           float fRollThresh) {
    Q_UNUSED(vHeartLat);
    Q_UNUSED(Norm);
...
#define SCALING 3.f
            ...
            if (anchor->vDir.z != 0 || anchor->vDir.x != 0) {
                anchorBase = glm::mat4(
                    glm::transpose(glm::mat4(-anchor->vDir.z / temp, 0.f, anchor->vDir.x / temp, 0.f, 0.f, 1.f, 0.f, 0.f,
                                             -anchor->vDir.x / temp, 0.f, -anchor->vDir.z / temp, 0.f, 0.f, 0.f, 0.f, 1.f)) *
                    glm::mat4(1.f, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f, -anchor->fPosHeartx(fHeart),
                              -anchor->fPosHearty(fHeart), -anchor->fPosHeartz(fHeart), 1.f));
            } else {
                anchorBase = glm::mat4(
                    glm::transpose(glm::mat4(-anchor->vNorm.z / temp, 0.f, anchor->vNorm.x / temp, 0.f, 0.f, 1.f, 0.f, 0.f,
                                             -anchor->vNorm.x / temp, 0.f, -anchor->vNorm.z / temp, 0.f, 0.f, 0.f, 0.f, 1.f)) *
                    glm::mat4(1.f, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f, -anchor->fPosHeartx(fHeart),
                              -anchor->fPosHearty(fHeart), -anchor->fPosHeartz(fHeart), 1.f));
            }
            ...
            KP1 = anchorBase *
                  (glm::vec4(glm::vec3(lNodes[lasti].vPosHeart(fHeart) + fThreshold / SCALING * lNodes[lasti].vDirHeart(fHeart)),
                             1.f));
            ...
            // fRollSpeedPerMeter = lNodes[(i)->fDistFromLast > 0.f ? lNodes[(i)].fRollSpeed/F_HZ/lNodes.at(i)]-fDistFromLast :
            // 0.f;
            KP2 = anchorBase *
                  (glm::vec4(glm::vec3(lNodes[i].vPosHeart(fHeart) - fThreshold / SCALING * lNodes[i].vDirHeart(fHeart)), 1.f));
            ...
            if (fabs(V.y) < fRollThresh) {
                ...
                glm::vec3 rotated =
                    glm::vec3(glm::rotate(glm::angle(lNodes[lasti].vDirHeart(fHeart), lNodes[i].vDirHeart(fHeart)), rotateAxis) *
                              glm::vec4(lNodes[lasti].vLatHeart(fHeart), 0.f));
...
void section::iFillPointList(QList<int>& List, float mPerNode) {
    ...
    for (int i = 1; i < lNodes.size(); i++) {
        lNodes[i].updateNorm();
        fThreshold +=
            lNodes[i].fDistFromLast;  // glm::distance(lNodes[(i)->fPosHeart(fHeart), lNodes.at(i-1)]-fPosHeart(fHeart));
        // qDebug("Node %d processed: Threshold is now %f", i, fThreshold);
        if (i == this->lNodes.size() - 1 || fThreshold > this->length / numNodes) {
    ...
    if (List.size() > 1 && (lNodes[List.last() - parent->getNumPoints(this) + 1].fTotalLength -
                                lNodes[List[List.size() - 2] - parent->getNumPoints(this) + 1].fTotalLength <
                            mPerNode / 2.f)) {
...
void section::calcDirFromLast(int i) {
    ...
    float fTrackYawFromLast =
        180.f / F_PI * (glm::atan(-curDirHeart.x, -curDirHeart.z) - glm::atan(-prevDirHeart.x, -prevDirHeart.z));
    float temp = cos(fabs(asin(curDirHeart.y)));
    lNodes[i].fTrackAngleFromLast =
        sqrt(temp * temp * fTrackYawFromLast * fTrackYawFromLast + fTrackPitchFromLast * fTrackPitchFromLast);

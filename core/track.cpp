...
void track::removeSmooth(int fromNode) {
    ...
    for (int i = 0; i < lSections.size(); ++i) {
        ...
        if (fromNode != 0)
            ...
            curNode = &lSections[i - 1]->lNodes.last();
        else
...
void track::updateTrack(int index, int iNode) {
    ...
    if (mParent->mMesh != NULL) mParent->mMesh->buildMeshes(nodeAt);

    mSec = timer.nsecsElapsed() / 1000000.;
    gloParent->showMessage(QString::number(mSec).append(QString("ms used to update %1 (%2) points").arg(count2).arg(count)),
                           3000);
...
int track::exportTrack2(fstream* file, float mPerNode, int fromIndex, int toIndex, float fRollThresh) {
    ...
    KP2_this =
        anchor->vDirHeart(fHeart) * glm::distance(anchor->vPosHeart(fHeart), getPoint(exportPoints[0])->vPosHeart(fHeart)) / 3.f;
    ...
    glm::vec3 startPoint = getPoint(exportPoints[0])->vPosHeart(fHeart) +
                           (KP2_this + anchorPos - getPoint(exportPoints[0])->vPosHeart(fHeart)) / 2.f * 3.f;
    qDebug("startPoint %f %f %f", startPoint.x, startPoint.y, startPoint.z);

    P = glm::vec3((1 / 6.f) * (startPoint + 4.f * getPoint(exportPoints[0])->vPosHeart(fHeart) +
                               getPoint(exportPoints[1])->vPosHeart(fHeart))) -
        anchorPos;
    KP1_this = glm::vec3((1 / 3.f) * (startPoint + 2.f * getPoint(exportPoints[0])->vPosHeart(fHeart))) - anchorPos;
    KP2_this = glm::vec3((1 / 3.f) *
                         (2.f * getPoint(exportPoints[0])->vPosHeart(fHeart) + getPoint(exportPoints[1])->vPosHeart(fHeart))) -
               anchorPos;
    ...
    if (fabs(V.y) > fRollThresh) {
        ...
        glm::vec3 rotated =
            glm::vec3(glm::rotate(glm::angle(anchor->vDirHeart(fHeart), V), rotateAxis) * glm::vec4(vLastLat, 0.f));
    ...
    for (i = 1; i < exportPoints.size() - 2; ++i) {
        ...
        P = glm::vec3((1 / 6.f) *
                      (getPoint(exportPoints[i - 1])->vPosHeart(fHeart) + 4.f * getPoint(exportPoints[i])->vPosHeart(fHeart) +
                       getPoint(exportPoints[i + 1])->vPosHeart(fHeart))) -
            anchorPos;
        KP1_this = glm::vec3((1 / 3.f) * (getPoint(exportPoints[i - 1])->vPosHeart(fHeart) +
                                          2.f * getPoint(exportPoints[i])->vPosHeart(fHeart))) -
                   anchorPos;
        KP2_this = glm::vec3((1 / 3.f) * (2.f * getPoint(exportPoints[i])->vPosHeart(fHeart) +
                                          getPoint(exportPoints[i + 1])->vPosHeart(fHeart))) -
                   anchorPos;
        ...
        if (fabs(V.y) > fRollThresh) {
            ...
            glm::vec3 rotated =
                glm::vec3(glm::rotate(glm::angle(getPoint(exportPoints[i - 1])->vDirHeart(fHeart), V), rotateAxis) *
                          glm::vec4(vLastLat, 0.f));
            temp = glm::angle(rotated, vHeartLat) * F_PI / 180.f;
            if (glm::dot(glm::cross(rotated, vHeartLat), V) > 0) {
                temp *= -1.f;
            }
    ...
    glm::vec3 endPoint = getPoint(exportPoints[exportPoints.size() - 1])->vPosHeart(fHeart) -
                         getPoint(exportPoints[exportPoints.size() - 1])->vDirHeart(fHeart) *
                             glm::distance(getPoint(exportPoints[exportPoints.size() - 1])->vPosHeart(fHeart),
                                           getPoint(exportPoints[exportPoints.size() - 2])->vPosHeart(fHeart));
    qDebug("endPoint %f %f %f", endPoint.x, endPoint.y, endPoint.z);

    P = glm::vec3((1 / 6.f) * (getPoint(exportPoints[i - 1])->vPosHeart(fHeart) + 4.f * endPoint +
                               getPoint(exportPoints[i + 1])->vPosHeart(fHeart))) -
        anchorPos;
    ...
    if (fabs(V.y) > fRollThresh) {
        ...
        glm::vec3 rotateAxis = glm::cross(getPoint(exportPoints[i - 1])->vDirHeart(fHeart), V);
        glm::vec3 rotated = glm::vec3(glm::rotate(glm::angle(getPoint(exportPoints[i - 1])->vDirHeart(fHeart), V), rotateAxis) *
                                      glm::vec4(vLastLat, 0.f));
        temp = glm::angle(rotated, vHeartLat) * F_PI / 180.f;
        if (glm::dot(glm::cross(rotated, vHeartLat), V) > 0) {
            temp *= -1.f;
    ...
    KP1_this = P - getPoint(exportPoints[exportPoints.size() - 1])->vDirHeart(fHeart) *
                       glm::distance(getPoint(exportPoints[exportPoints.size() - 1])->vPosHeart(fHeart),
                                     getPoint(exportPoints[exportPoints.size() - 2])->vPosHeart(fHeart)) /
                       3.f;
...
void track::exportNL2Track(FILE* file, float mPerNode, int fromIndex, int toIndex) {
    ...
    glm::mat3 anchorBase = glm::transpose(glm::mat3(-anchor->vDir.z / temp, 0.f, anchor->vDir.x / temp, 0.f, 1.f, 0.f,
...
mnode* track::getPoint(int index) {
    ...
    if (lSections.size() == i) {
        if (lSections.size())
            return &lSections.last()->lNodes.last();
        else
            return anchorNode;
    }
